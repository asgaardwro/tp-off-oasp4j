...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriod != null) {
      ...
      int hour = activePeriod.getStartingHour();
      query.where(Alias.$(special.getActivePeriod().getStartingHour()).eq(hour));

      hour = activePeriod.getEndingHour();
      query.where(Alias.$(special.getActivePeriod().getEndingHour()).eq(hour));
